...
export default function UpcomingMoviesContent() {

    const { data: upcomingMovies, error: errorUpcomingMovies, isLoading } = useUpcomingMovies();

    const navigate = useNavigate();

    if (isLoading) return <p>Cargando...</p>;
    if (errorUpcomingMovies) return <p>Error: {errorUpcomingMovies.message}</p>;

    return (
        <main className="w-full animate-fade-up">
            <h1 className='text-4xl text-start mb-10'>Próximos estrenos</h1>
            <div className='grid gap-3 grid-cols-[repeat(auto-fit,_minmax(250px,_1fr))]'>
                {
                    upcomingMovies?.map((movie: UpcomingMoviesInterface) => {
                        return (
